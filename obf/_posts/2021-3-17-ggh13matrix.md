...
In this section we present main cryptographic tools to be used in the scheme. We do not introduce GGH13 multilinear map and obfuscation scheme, which have many details and are well described in previous papers [1].

#### Encrypting matrices

Matrices of the same rank may be encrypted with perfect security, thanks to lemma proven by Ishai and Kushilevits [2].

**Lemma** Let $\mathbb{Z}_q$ be a finite field. For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_q)$ and uniformly random invertible matrices $R,~S \in M_{k \times k}(\mathbb{Z}_q)$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
