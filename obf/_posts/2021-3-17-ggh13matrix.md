 ...
 **Abstract.** We propose a generalization of the obfuscation scheme based on GGH13 multilinear map \cite{ggh13}, which security is based on the hardness of NTRU problem. In 2014 Bernstein et al. proposed NTRU Prime scheme \cite{ntru-prime}, which in contradiction to NTRU Classic \cite{ntru-classic} and NTRU NTT (used for example in \cite{ntru-ntt}) uses irreducible polynomial to create quotient ring. It enables us to represent plaintext program as a sequence of matrices over a finite field in our scheme. To encrypt matrices over a field we use perfectly secure encryption proposed by Ishai and Kushievitz \cite{ishai} (also used for program obfuscation with ADP representation by Bartusek et al. \cite{adp}). We use specially designed block matrices to annihilate noisy terms during evaluation of encrypted program. The security of the scheme is based on the matrix version of NTRU Problem and perfect security of encryption method. The efficiency of the scheme is derived from NTRU Prime efficiency, which uses Karatsuba multiplication to gain speed comparable to NTT implementations.

### Introduction

Obfuscation is focused on encrypting functions and using noncommutative algebraic structures seem to be natural approach to achieve that goal. Linearized functions may be presented as a product of matrices. In known solutions each matrix encrypts the value of fixed variable and the function to be performed on that value. We give to the user matrices encrypting different values (typically bit values 0 or 1 for each input binary variable of obfuscated program), which he may choose while executing the encrypted program. User can choose only values, encrypted function is determined. At the end of computations, thanks to public parameters of obfuscation scheme, user may decrypt the result.

### Preliminaries

In this section we present main cryptographic tools to be used in the scheme. We do not introduce GGH13 multilinear map and obfuscation scheme, which have many details and are well described in previous papers \cite{ggh13}.

#### Encrypting matrices

Matrices of the same rank may be encrypted with perfect security, thanks to lemma proven by Ishai and Kushilevits \cite{ishai}.

**Lemma** Let $\mathbb{Z}_q$ be a finite field. For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_q)$ and uniformly random invertible matrices $R,~S \in M_{k \times k}(\mathbb{Z}_q)$ the distribution of a product $R \cdot A \cdot S$ depend only on the rank of matrix $A$.
...
Please notice, that for random invertible matrices $Z_i$ and $Z_{i+1}$ we use encryption method proposed by Ishai and Kushilevitz \cite{ishai}. However, while for the numbers encryption is multiplicatively homomorphic, it is not true for matrices as:
...
$$
  =
  \left(\begin{array}{ll}
    0 & \g R_{12} G \cdot R_{24} G\\
    \y M_1 \cdot M_2 & \g M_1 \cdot R_{22}G + R_{14}G \cdot R_{24}G\\
  \end{array}\right)
=
  \left(\begin{array}{cc}
    0 & \g R_{p22} G\\
    \y M_1 \cdot M_2 & \g R_{p24}G\\
  \end{array}\right),
$$
...
$$
  \left(\begin{array}{cc}
    0 & \g R_{p(K-1)2} G\\
    \y \prod_{i=1}^{K-1} M_i & \g R_{p(K-1)4}G\\
  \end{array}\right)
  \cdot
  \left(\begin{array}{cc}
    \e R_x & \y R_{K2} G + M_K\\
...
$$
  =
  \left(\begin{array}{cc}
    \e R_{p(K-1)2} G \cdot R_y & \g R_{p(K-1)2} G \cdot R_{K4} G \\
    \e \prod_{i=1}^{K-1} M_i \cdot R_x + R_{p(K-1)4}G \cdot R_y & \y \prod_{i=1}^{K-1} M_i \cdot R_{K2} G + \prod_{i=1}^{K} M_i + R_{p(K-1)4}G \cdot R_{K4} G\\
  \end{array}\right) =
$$
...
$$
  ...
  \left(\begin{array}{cc}
    \e R_{pK1} & \g R_{pK2} G \\
    \e R_{pK3} & \y R_{pK4} G + \prod_{i=1}^{K} M_i \\
  \end{array}\right).
$$
...
**Lemma 1** Let $p$ be a prime number and $\mathbb{Z}_{p^n}$ be an extended finite field. For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_{p^n})$ and uniformly random invertible matrices $R,~S \in M_{k \times k}(\mathbb{Z}_{p^n})$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
...
\begin{multline*}
    W = U \cdot p_{zt} = Z_0 \cdot \left (R \cdot encode(G) + \prod M'_i \right) encode(G^{-1}) \cdot H = \\
...
\begin{enumerate}
    \item $C_4$ belongs to ideal $I = \langle G \rangle$,
    \item $C_4$ represents nonzero coset of ideal $I$.
\end{enumerate}

We would like to show, that in case 1:
    $$||W||_{\infty}
    = ||Z_0 \cdot C \cdot encode(G^{-1}) \cdot H ||_{\infty} \leqslant q^{\frac{3}{4}}$$
...
$$ ||Z_0 \cdot C \cdot encode(G^{-1}) \cdot H ||_{\infty} \leqslant
    poly(k) \cdot poly(n) \cdot ||Z_0 ||_{\infty} \cdot ||C \cdot encdoe(G^{-1})||_{\infty} \cdot ||H||_{\infty} \stackrel{\textcircled{\tiny{1}}}{\leqslant}$$

$$ \stackrel{\textcircled{\tiny{1}}}{\leqslant}  poly(k) \cdot poly(n) \cdot
    ||Z_0 ||_{\infty} \cdot ||C||_{\infty} \cdot ||H||_{\infty}, $$

where $k$ is matrix dimension and $n$ is a degree of polynomial used for construction of ring $R_q$. If we choose random matrices $Z_0$ and $H$ having norm smaller than $q^{\frac{1}{4}}$ and if we assume that the norm of ciphertext $C$ must be always lower than $q^{\frac{1}{8}}$ (from the construction), then the norm of $W$ is lower than $q^{\frac{5}{8}}$, which is lower than $q^{\frac{3}{4}}$ with additional margin. Inequality $\textcircled{\tiny{1}}$ follows from the fact that if $C_2$ and $C_4$ lie in the ideal generated by $G$, then they have a forms: $C_2 = C_2'\cdot G$ and $C_4 = C_4'\cdot G$ for some matrices $C_2'$ and $C_4'$ respectively. In consequence:
...
$$ || C \cdot encode(G^{-1})||_{\infty} = ||C'||_{\infty} \leqslant  ||C' \cdot encode(G)||_{\infty} = ||C||_{\infty}, $$

because both norms $||C_2'||_{\infty}$, $||C_4'||_{\infty}$ and $||encode(G)||_{\infty}$ are much lower then $q$ and there are no modular reductions during multiplication.
...
We give a sketch of the prove for Lemma \ref{lem:elems-prove} similar to lemma in GGH13 multilinear map. We assume that matrices being encrypted are permutation matrices (which has only binary elements 0 or 1) multiplied by integer safeguards (see Section \ref{sec:security}). In consequence matrix elements are either 0 or $t$, for some integer~$t$.
...
**Lemma 2** Suppose $m_{ij} \in \{0,t\}$ for positive integer $t$ and suppose that
    ...
    for $1 \leqslant k \leqslant l^2$ and $||w_k g||$ and $||\sum_{i=1}^l \sum_{j=1}^l z_{kj} \cdot m_{ji} \cdot h_{ik}||$ are smaller than $q/2$, for random elements $z_{kj}$ and $h_{ik}$ that does not belong to $\langle g \rangle$. Then with overwhelming probability all elements $m_{ij}$ belong to ideal $\langle g \rangle$.
...
**Proof** Assuming that both norms are smaller than $q/2$ the equation holds over $R$, not only over $R_q$. Let's assume that there exists an element $m_{rs}$ that does not belong to $\langle g \rangle$.
    ...
    3. To answer the question we consider lattice connected with ideal $\langle g \rangle$ and its determinant (volume of its fundamental domain), which determines the density of the lattice elements in the entire space. According to \cite{lattice-book} the determinant of lattice $L$ given by basis $\mathcal{B}$ may be computed as follows:

    $$ det(L(\mathcal{B})) = \sqrt{det(B^TB)},$$
    ...
    for right choice of generator g, with one coefficient significantly bigger then others, we can estimate the size $det(B) \approx ||g||_{\infty}^n$.
    ...
    $$ Pr = \frac{1}{det(B)} \approx ||g||_{\infty}^{-n}.$$
    ...
    $$ Pr_{m} = \left ( \frac{1}{det(B)} \right )^{l^2} \approx ||g||_{\infty}^{-n \cdot l^2}.$$

    The probability depends on matrix dimension $l$ and chosen ideal generator $g$.
    Using Barrington Theorem \cite{barrington} we use matrices of dimension $l=5$, so $l^2=25$. Even for very small determinant $det(L(\mathcal{B}))=2$ the probability of error is lower than $2^{-25}$. For the right choice of the generator $g$, probability exponentially tends to 0 with its degree $n$ tending to infinity.

In accordance with Lemma 2 if $||w_k g||$ and $||\sum_{i=1}^l \sum_{j=1}^l z_{kj} \cdot m_{ji} \cdot h_{ik}||$ are smaller than $q/2$, elements $m_{ij}$ belong to ideal with overwhelming probability. If elements does not belong to ideal, then with overwhelming probability either $||w_k g||>q/2$ or $||\sum_{i=1}^l \sum_{j=1}^l z_{kj} \cdot m_{ji} \cdot h_{ik}||>q/2$. We will show taht in the first cases norm of ciphertext $||w_k||>q^{3/4}$ and in consequence $||W_M||>q^{3/4}$, while the second case is contradictory with assumption on the ciphertext size.
...
2. Assume that $||\sum_{i=1}^l \sum_{j=1}^l z_{kj} \cdot m_{ji} \cdot h_{ik}||>q/2$. Then:
  ...
  $$ ||C||_{\infty} > max_{i,j}|| m_{ji} || > \frac{q/2}{l^2 \cdot poly(n) \cdot max_{j,k}||z_{kj}|| \cdot max_{i,k}|| h_{ik}||} \approx q^{1/2}, $$
