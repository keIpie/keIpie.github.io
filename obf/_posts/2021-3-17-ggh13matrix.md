...
$$ \begin{multline*}
    ...
    = Z_{i-1} \cdot \begin{bmatrix} \y (M_{i0}-M_{i1}) & \g (B_{i0}-B_{i1}) G \\
    \overline{0} &  \g (D_{i0}-D_{i1}) G
...
The efficiency of a scheme depends on the efficiency o multiplying matrices over polynomial ring, consisting in many parallel polynomial multipliactions. In this scheme, we propose to use parameters such as in Streamlined NTRU Prime: the ring $R = \mathbb{Z}_q[x]/(x^p-x-1)$ (isomorphic to finite field $\mathbb{Z}_{q^p}$) for $q=4591$ and $p=761$. The efficiency of computations in the ring $R$ is described in detail in the paper \cite{ntru-prime}. Authors used Karatsuba multiplication instead of NTT-based multiplication, getting results comparable with NTT implementations.
