...
Let $Z_q$ be a finite field. For arbitrary matrix $A \in M_{k \times k}(Z_q)$ and uniformly random invertible matrices
$R,~S \in M_{k \times k}(Z_q)$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
...
**Lemma 1**
 Let $p$ be a prime number and $\mathbb{Z}_{p^n}$ be an extended finite field.
 For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_{p^n})$ and uniformly random invertible matrices
 $R,~S \in M_{k \times k}(\mathbb{Z}_{p^n})$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
...
for right choice of generator g, with one coefficient significantly bigger then others, we can estimate the size $det(B) \approx ||g||^n_{\infty}$.
...
$$ Pr = \frac{1}{det(B)} \approx ||g||^{-n}_{\infty}.$$
...
$$ Pr_{m} = \left ( \frac{1}{det(B)} \right )^{l^2} \approx ||g||^{-n \cdot l^2}_{\infty}.$$
