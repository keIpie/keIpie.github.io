...
Let $\mathbb{Z}_q$ be a finite field. For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_q)$ and uniformly random invertible matrices $R,~S \in M_{k \times k}(\mathbb{Z}_q)$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
