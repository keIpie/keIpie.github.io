...
**Lemma 1**
 ...
 Let $p$ be a prime number and $\mathbb{Z}_{p^n}$ be an extended finite field. For arbitrary matrix $A \in M_{k \times k}(\mathbb{Z}_{p^n})$ and uniformly random invertible matrices $R,~S \in M_{k \times k}(\mathbb{Z}_{p^n})$ the distribution of a product $R \cdot A \cdot S$ depends only on the rank of matrix $A$.
...
Suppose $m_{ij} \in \{0,t\}$ for positive integer $t$ and suppose that:
...
Assuming that both norms are smaller than $q/2$ the equation holds over $R$, not only over $R_q$. Let's assume that there exists an element $m_{rs}$ that does not belong to $\langle g \rangle$.
