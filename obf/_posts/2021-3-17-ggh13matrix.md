...
smaller than $q/2$, elements $m_{ij}$ belong to ideal with overwhelming probability. If elements does not belong to ideal,
then with overwhelming probability either $||w_k g||>q/2$ or $||\sum^l_{i=1} \sum^l_{j=1} z_{kj} \cdot m_{ji} \cdot h_{ik}||>q/2$.
We will show taht in the first cases norm of ciphertext $||w_k||>q^{3/4}$ and in consequence $||W_M||>q^{3/4}$, while the second
case is contradictory with assumption on the ciphertext size.
