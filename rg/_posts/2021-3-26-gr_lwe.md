...
są postaci $(\|f(\eta^i)\| \pm \|g(\eta^i)\|)^2$  dla $\eta=e^{2 \pi i/n}$ (n-ty pierwiastek z jedynki), a norma jest normą w $\mathbb{C}$.
Stąd norma $h$ jest ograniczona z góry przez wartość $max\{ \|f(\eta^i)\| \pm \|g(\eta^i)\| ~|~i=0,1,\ldots n-1 \}$.
