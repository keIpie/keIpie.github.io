...
Możemy obliczyć $a$ zapisując współczynniki $z$ modulo $q$ w bazie $B$. Nie możemy po prostu zapisać współczyników $z$, bo nie wiemy
...
$$ b = z(x + e)^T = zx^T + ze^T \mod{qZ},$$

oraz $s = xB^T$. Zauważmyh, że:

$$ \| ze^T \|_{\infty} \leqslant m \beta, $$

$$ zx^T = aBx^T = aB(s(B−1)^T)^T = as.$$

Wyrocznia R-LWE może zwrócić $s$, na podstawie którego można obliczyć $x \mod{qL^{\star}}, co dopełnia redukcję.
