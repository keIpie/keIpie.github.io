...
jest dobrze zdefiniowany i może być zrzutowany na $\bigoplus_{i=2}^{(n+1)/2} \mathbb{C}^{2 \times 2}$.
...
Niech $L$ i $L^{-1}$ będą kratami zdefiniowanymi przez zanurzenie współczynników $I$ i $I^{-1}. Wtedy $L^{\star}$
