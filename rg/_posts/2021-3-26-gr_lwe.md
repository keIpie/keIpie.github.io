...
**Twierdzenie** Mnożenie elementów w $\mathbb{Z}_q[D_{2n}]$ można wykonać ze złożonością czasową $\tilde{O}(n \log{q})$.

**Dowód** Niech $f_1 + sf_2$ i $f_3 + sf_4$ będą dwoma elementami $\mathbb{Z}_q[D_{2n}]$, gdzie $f_1,f_2,f_3,f_4$ są wielomianami zmiennej $t$.
Iloczyn elementów będzie postaci:

$$ (f_1 + sf_2) \cdot (f_3 + sf_4) =  f_1f_3 + sf_2f_3 + f_1sf_4 + sf_2sf_4 =f_1f_3 + sf_2f_3 + s(sf_1s)f_4 + (sf_2s)f_4
