...
2. Redukcja problemu DGS na kracie $L$ do problemu $\beta-BDD$ (znajdywania najbliższego wektora kraty, jeżeli wiemy, że
  leży on w odległości nie dalszej niż $\beta$) na kracie dualnej $L^{\star}$. Ta redukcja zakład użycie algorytmu kwantowego
  (nie jest możliwa na komputerze klasycznym).  

3. Sprowadzenie problemu $\beta-BDD$ do $(q,\beta)-BDD$. Parametr $q$ określa normę euklidesową $l_q$.

4. Sprowadzenie problemu $(q,\beta)-BDD$ do problemu DGS dla rozkładu o większej szerokości. **Ten krok wymaga wyroczni LWE**.
